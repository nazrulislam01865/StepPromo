                    ...
                    @forelse($jobs as $job)
                        ...
                        @elseif($sequence === 5)
                            <tr class="{{ $rowClass }}" style="{{ $rowStyle }}" x-data x-on:click="window.location.href='{{ $detailUrl }}'">
                                ...
                                <td data-label="Tracking"><b>{{ data_get($row,'tracking') }}</b>@if(data_get($row,'tracking') === 'Pending')<div class="cell-controls"><a class="cell-action" href="{{ $detailUrl }}" wire:navigate x-on:click.stop>Dispatch shipment</a></div>@endif</td>
